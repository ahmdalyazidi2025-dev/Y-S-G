const admin = require('firebase-admin');

const serviceAccount = {
    "type": "service_account",
    project_id: "y-s-g-7c463",
    "private_key_id": "de5410939a349c7cc89dec5d740193025da3bc1f",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "firebase-adminsdk-fbsvc@y-s-g-7c463.iam.gserviceaccount.com"
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount)
    });
}

const db = admin.firestore();

async function run() {
    const snap = await db.collection("products").get();
    console.log("Total Products in Firestore:", snap.docs.length);

    let expired = 0;
    let active = 0;
    let activeNoDiscount = 0;
    const now = new Date();

    snap.docs.forEach(doc => {
        const data = doc.data();
        let isExpired = false;
        if (data.discountEndDate) {
            let date;
            if (data.discountEndDate.toDate) {
                date = data.discountEndDate.toDate();
            } else {
                date = new Date(data.discountEndDate);
            }
            if (date < now) {
                isExpired = true;
            }
        }
        if (isExpired) {
            expired++;
        } else {
            active++;
        }
    });

    console.log("Active products (not expired):", active);
    console.log("Expired products:", expired);
}

run();
