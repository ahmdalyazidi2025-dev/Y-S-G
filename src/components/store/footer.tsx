            ...
            <div className="container mx-auto px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
                    <FeatureBox
                        ...
                        title={storeSettings?.shippingTitle || "الشحن مجاناً"}
                        desc={storeSettings?.shippingDesc || "لكل الطلبات فوق حد معين"}
                        iconColor="text-blue-500"
                        bgColor="bg-blue-500/10"
                    />
                    <FeatureBox
                        icon={<ShieldCheck className="w-6 h-6" />}
                        title={storeSettings?.paymentTitle || "الدفع السهل"}
                        desc={storeSettings?.paymentDesc || "طرق دفع متنوعة وآمنة"}
                        iconColor="text-yellow-500"
                        bgColor="bg-yellow-500/10"
                    />
                    <FeatureBox
                        icon={<Headphones className="w-6 h-6" />}
                        title={storeSettings?.supportTitle || "دعم فني"}
                        desc={storeSettings?.supportDesc || "متواجدون لخدمتكم دائماً"}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        <h3 className="text-foreground font-bold text-xl border-r-4 border-primary pr-4">
                            {storeSettings?.aboutTitle || "عن المتجر"}
                        </h3>
                        <p className="text-sm leading-relaxed text-foreground dark:text-slate-400 text-right">
                            {storeSettings?.aboutText || "وصف المتجر والخدمات المقدمة."}
                        </p>
                    </div>
                ...
                <div className="pt-8 border-t border-border flex flex-col md:flex-row items-center justify-between gap-8">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 md:gap-12 items-center">
                        <div className="flex items-center gap-3 bg-white dark:bg-white/5 px-4 py-2 rounded-2xl border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none">
                            <div className="text-right">
                                ...
                                <p className="text-sm font-bold text-foreground dark:text-white tracking-widest">{storeSettings?.contactPhone || "غير مسجل"}</p>
                            </div>
                        ...
                        <div className="flex items-center gap-3 text-sm">
                            <span className="text-foreground">{storeSettings?.contactAddress || "العنوان غير محدد"}</span>
                    ...
                    <div className="text-xs text-foreground dark:text-slate-500">
                        © {new Date().getFullYear()} {storeSettings?.aboutTitle || "المتجر"}. جميع الحقوق محفوظة.
                    </div>
                </div>
            </div>
