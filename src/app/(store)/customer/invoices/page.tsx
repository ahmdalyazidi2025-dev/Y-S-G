                                ...
                                <div className="flex items-center justify-between py-2 border-y border-white/5">
                                    <div className="flex -space-x-2 rtl:space-x-reverse">
                                        {order.items.slice(0, 3).map((item, idx) => (
                                            <div key={idx} className="w-8 h-8 rounded-full bg-white/10 border-2 border-[#1c2a36] flex items-center justify-center text-[10px] overflow-hidden">
                                        ...
                                        <div className="space-y-3">
                                            {selectedOrder.items.map((item: any, idx: number) => (
                                                <div key={idx} className="glass-card p-4 flex items-center gap-4">
                                                    <div className="w-12 h-12 bg-white/5 rounded-xl border border-white/5 flex items-center justify-center text-xs overflow-hidden">
                                                        {item.image ? <img src={item.image} alt="" className="w-full h-full object-cover" /> : item.name.charAt(0)}
                                                    </div>
                                                    <div className="flex-1">
                                                        <p className="font-bold text-sm text-white">{item.name}</p>
                                                        <p className="text-[10px] text-slate-500">{item.quantity} {item.unit} × {item.price} ر.س</p>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="font-bold text-sm text-primary">{(item.quantity * item.price).toFixed(2)}</p>
