...
import type { Message, Conversation } from "@/context/store-context"
...
export default function AdminChatPage() {
    ...
    const conversations = useMemo(() => {
        ...
        messages.forEach(m => {
            if (m.senderId === "admin") {
                // For admin messages, we don't know the recipient from the message itself in this simple mock
                // In a real app, messages would have a recipientId
                return
            }
            if (!convs[m.senderId]) {
                convs[m.senderId] = {
        ...
        // Add customers who haven't messaged yet
        customers.forEach(c => {
            if (!convs[c.id]) {
                convs[c.id] = {
                    customerId: c.id,
    ...
    const handleSend = () => {
        ...
        if (mode === "broadcast") {
            ...
            // Append target customer ID to mock "routing"
