const admin = require('firebase-admin');
const serviceAccountData = {
    "type": "service_account",
    "project_id": "y-s-g-7c463",
    "private_key_id": "de5410939a349c7cc89dec5d740193025da3bc1f",
    "private_key": "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n",
    "client_email": "firebase-adminsdk-fbsvc@y-s-g-7c463.iam.gserviceaccount.com"
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(serviceAccountData)
    });
}
const db = admin.firestore();

async function findCustomer() {
    console.log("Searching customers by phone 0539893200...");
    const snap = await db.collection("customers").where("phone", "==", "0539893200").get();
    console.log(`Found ${snap.size} customers:`);
    snap.docs.forEach(doc => {
        console.log(doc.id, JSON.stringify(doc.data(), null, 2));
    });
    
    console.log("\nSearching usernames collection for 'الدانه10'...");
    const udoc = await db.collection("usernames").doc("الدانه10").get();
    console.log("Username exists?", udoc.exists, udoc.data());
    
    // Also search users collection
    const usnap = await db.collection("users").where("phone", "==", "0539893200").get();
    console.log(`Found ${usnap.size} users:`);
    usnap.docs.forEach(doc => {
        console.log(doc.id, JSON.stringify(doc.data(), null, 2));
    });

    process.exit(0);
}
findCustomer();
