const admin = require('firebase-admin');
const serviceAccountData = {
    "type": "service_account",
    "project_id": "y-s-g-7c463",
    "private_key_id": "de5410939a349c7cc89dec5d740193025da3bc1f",
    "private_key": "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n",
    "client_email": "firebase-adminsdk-fbsvc@y-s-g-7c463.iam.gserviceaccount.com"
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(serviceAccountData)
    });
}
const db = admin.firestore();

async function inspect() {
    const snap = await db.collection("orders").where("customerId", "==", "Vc4NpSQzyBObnSNdTWUg0e7qQcF3").get();
    console.log(`Found ${snap.size} orders for Vc4NpSQzyBObnSNdTWUg0e7qQcF3:`);
    snap.docs.forEach(doc => {
        console.log(doc.id, JSON.stringify(doc.data(), null, 2));
    });
    
    // Also check if there is a customer with ID Vc4NpSQzyBObnSNdTWUg0e7qQcF3
    const cust = await db.collection("customers").doc("Vc4NpSQzyBObnSNdTWUg0e7qQcF3").get();
    console.log("Customer doc Vc4NpSQzyBObnSNdTWUg0e7qQcF3 exists?", cust.exists, cust.data());
    
    process.exit(0);
}
inspect();
